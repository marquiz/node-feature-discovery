...
> **NOTE:** unprefixed labels (like `foo=bar`) are not allowed but they can be
> used in the `local.feature` Feature in which case `# +no-label` directive
> should be used.
...
- unprefixed names (like `my-annotation`) are not allowed
...
- the only exception is `feature.node.kubernetes.io/` and its sub-namespaces
  (like `sub.ns.feature.node.kubernetes.io`)
- unprefixed names (like `my-er`) are not allowed
