...
func readIfaceInfo(path string) feature.InstanceFeature {
	...
	info.Attributes["name"] = filepath.Base(path)

	for _, attrName := range ifaceAttrs {
		data, err := ioutil.ReadFile(filepath.Join(path, attrName))
		if err != nil {
			klog.V(3).Infof("failed to read net iface attribute %s: %v", attrName, err)
			continue
		}
		info.Attributes[attrName] = strings.TrimSpace(string(data))
	}

	for _, attrName := range devAttrs {
		data, err := ioutil.ReadFile(filepath.Join(path, "device", attrName))
		if err != nil {
			klog.V(3).Infof("failed to read net device attribute %s: %v", attrName, err)
			continue
		}
		info.Attributes[attrName] = strings.TrimSpace(string(data))
	}
