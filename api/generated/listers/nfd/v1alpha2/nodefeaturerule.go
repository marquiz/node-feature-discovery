...
type NodeFeatureRuleLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.NodeFeatureRule, err error)
	// Get retrieves the NodeFeatureRule from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.NodeFeatureRule, error)
...
// List lists all NodeFeatureRules in the indexer.
func (s *nodeFeatureRuleLister) List(selector labels.Selector) (ret []*v1alpha2.NodeFeatureRule, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.NodeFeatureRule))
	})
	return ret, err
}

// Get retrieves the NodeFeatureRule from the index for a given name.
func (s *nodeFeatureRuleLister) Get(name string) (*v1alpha2.NodeFeatureRule, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("nodefeaturerule"), name)
	}
	return obj.(*v1alpha2.NodeFeatureRule), nil
}
