...
type NodeFeatureGroupLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.NodeFeatureGroup, err error)
...
// List lists all NodeFeatureGroups in the indexer.
func (s *nodeFeatureGroupLister) List(selector labels.Selector) (ret []*v1alpha2.NodeFeatureGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.NodeFeatureGroup))
	})
	return ret, err
}
...
type NodeFeatureGroupNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.NodeFeatureGroup, error)
...
// List lists all NodeFeatureGroups in the indexer for a given namespace.
func (s nodeFeatureGroupNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.NodeFeatureGroup, err error) {
...
// Get retrieves the NodeFeatureGroup from the indexer for a given namespace and name.
func (s nodeFeatureGroupNamespaceLister) Get(name string) (*v1alpha2.NodeFeatureGroup, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("nodefeaturegroup"), name)
	}
	return obj.(*v1alpha2.NodeFeatureGroup), nil
}
