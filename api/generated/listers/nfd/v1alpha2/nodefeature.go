...
type NodeFeatureLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.NodeFeature, err error)
...
// List lists all NodeFeatures in the indexer.
func (s *nodeFeatureLister) List(selector labels.Selector) (ret []*v1alpha2.NodeFeature, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.NodeFeature))
	})
	return ret, err
}
...
type NodeFeatureNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.NodeFeature, error)
...
// List lists all NodeFeatures in the indexer for a given namespace.
func (s nodeFeatureNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.NodeFeature, err error) {
...
// Get retrieves the NodeFeature from the indexer for a given namespace and name.
func (s nodeFeatureNamespaceLister) Get(name string) (*v1alpha2.NodeFeature, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("nodefeature"), name)
	}
	return obj.(*v1alpha2.NodeFeature), nil
}
