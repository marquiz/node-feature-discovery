...
func (m *nfdWorker) updateNodeFeatureObject(labels Labels) error {
	...
	if nfr, err := cli.NfdV1alpha1().NodeFeatures(namespace).Get(context.TODO(), nodename, metav1.GetOptions{}); errors.IsNotFound(err) {
		...
		nfr.Annotations = map[string]string{nfdv1alpha1.WorkerVersionAnnotation: version.Get()}
		nfr.Labels = map[string]string{nfdv1alpha1.NodeFeatureObjNodeNameLabel: nodename}
		nfr.Spec = nfdv1alpha1.NodeFeatureSpec{
			Features: *features,
			Labels:   labels,
		}

		nfrUpdated, err := cli.NfdV1alpha1().NodeFeatures(namespace).Update(context.TODO(), nfr, metav1.UpdateOptions{})
		if err != nil {
