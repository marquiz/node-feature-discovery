...
func (m *nfdMaster) Run() error {
	...
	if m.args.FeatureRulesController {
		...
		m.nfdController = newNfdController(kubeconfig, !m.args.EnableNodeFeatureApi)
	}
...
func (m *nfdMaster) prune() error {
	...
	for _, node := range nodes.Items {
		...
		err := m.updateNodeObject(cli, node.Name, Labels{}, Annotations{}, ExtendedResources{}, nil)
		if err != nil {
...
func (m *nfdMaster) nfdAPIUpdateOneNode(nodeName string) error {
	...
	if len(objs) == 0 {
		...
		return fmt.Errorf("failed to get NodeFeature resources for %q: %w", nodeName, err)
	}
